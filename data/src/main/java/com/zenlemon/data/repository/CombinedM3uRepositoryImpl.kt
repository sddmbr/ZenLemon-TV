...
import com.zenlemon.data.local.dao.CombinedM3uProfileDao
import com.zenlemon.data.local.dao.CombinedM3uProfileMemberDao
import com.zenlemon.data.local.dao.ProviderDao
import com.zenlemon.data.local.entity.CombinedM3uProfileEntity
import com.zenlemon.data.local.entity.CombinedM3uProfileMemberEntity
...
import com.zenlemon.domain.model.CombinedCategoryBinding
            ...
            if (memberProviders.isEmpty()) {
                ...
                combine(categoryFlows) { arrays ->
                    arrays.toList()
                        .map { it as Triple<*, *, *> }
                        .flatMap { triple ->
                            val member = triple.first as com.zenlemon.domain.model.CombinedM3uProfileMember
                            val categories = triple.second as List<Category>
                            val hiddenCategoryIds = triple.third as Set<Long>
                            categories
                                .filter {
                                    !it.isVirtual &&
                                        it.id != com.zenlemon.domain.repository.ChannelRepository.ALL_CHANNELS_ID &&
