package com.zenlemon.data.manager
...
import com.zenlemon.data.local.dao.PlaybackHistoryDao
import com.zenlemon.data.local.dao.ProviderDao
import com.zenlemon.data.local.dao.RecordingScheduleDao
import com.zenlemon.data.local.dao.VirtualGroupDao
import com.zenlemon.data.local.entity.ProviderEntity
import com.zenlemon.data.local.entity.RecordingScheduleEntity
import com.zenlemon.data.mapper.toDomain
import com.zenlemon.data.mapper.toEntity
import com.zenlemon.data.preferences.ParentalPinBackupData
import com.zenlemon.data.preferences.PreferencesRepository
import com.zenlemon.data.security.CredentialCrypto
import com.zenlemon.domain.manager.BackupData
import com.zenlemon.domain.manager.BackupConflictStrategy
import com.zenlemon.domain.manager.BackupImportPlan
import com.zenlemon.domain.manager.BackupImportResult
import com.zenlemon.domain.manager.BackupManager
import com.zenlemon.domain.manager.BackupPreview
import com.zenlemon.domain.manager.RecordingScheduleImportDisposition
import com.zenlemon.domain.manager.RecordingScheduleImportOutcome
import com.zenlemon.domain.manager.RecordingScheduleImportSummary
import com.zenlemon.domain.manager.ProtectedCategoryBackup
import com.zenlemon.domain.manager.RecordingManager
import com.zenlemon.domain.manager.ScheduledRecordingBackup
...
class BackupManagerImpl @Inject constructor(
    ...
    override suspend fun exportConfig(uriString: String): com.zenlemon.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            } ?: return@withContext com.zenlemon.domain.model.Result.error("Failed to open output stream")

            com.zenlemon.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            com.zenlemon.domain.model.Result.error("Failed to export backup: ${e.message}", e)
        }
    }
    ...
    ): com.zenlemon.domain.model.Result<BackupImportResult> = withContext(Dispatchers.IO) {
        try {
            val backupData = readBackupData(uriString)
                ?: return@withContext com.zenlemon.domain.model.Result.error("Failed to open input stream")

            if (backupData.version > CURRENT_BACKUP_VERSION) {
                return@withContext com.zenlemon.domain.model.Result.error("Unsupported backup version")
            }
            if (backupData.isStructurallyEmpty()) {
                return@withContext com.zenlemon.domain.model.Result.error("Backup file does not contain any importable data")
            }
            if (!verifyChecksum(backupData)) {
                return@withContext com.zenlemon.domain.model.Result.error("Backup file is corrupted (checksum mismatch)")
            }
            ...
            com.zenlemon.domain.model.Result.success(
                BackupImportResult(
            ...
            com.zenlemon.domain.model.Result.error("Failed to import backup: ${e.message}", e)
        }
    }
    ...
    private suspend fun restorePreferences(prefs: Map<String, String>) {
        ...
        prefs["appLandingDestination"]?.takeIf { it.isNotBlank() }?.let { savedDestination ->
            preferencesRepository.setAppLandingDestination(
                com.zenlemon.domain.model.AppLandingDestination.fromStorage(savedDestination)
            )
        }
        ...
        val legacyDecoderMode = prefs["playerDecoderMode"]
            ...
            ?.let { savedMode ->
                com.zenlemon.domain.model.DecoderMode.entries
        ...
        prefs["playerAudioOutputPreference"]?.takeIf { it.isNotBlank() }?.let { savedPreference ->
            val preference = com.zenlemon.domain.model.AudioOutputPreference.entries
        ...
        prefs["playerSurfaceMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val surfaceMode = com.zenlemon.domain.model.PlayerSurfaceMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (surfaceMode != null) {
        ...
        prefs["playerLiveStreamFormatMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val formatMode = com.zenlemon.domain.model.LiveStreamFormatMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (formatMode != null) {
        ...
        (prefs["playerVodHttpProtocolMode"] ?: prefs["playerMovieHttpProtocolMode"])?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val protocolMode = com.zenlemon.domain.model.VodHttpProtocolMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
    ...
    private suspend fun restoreSavedLibrary(
        ...
        val categoriesByProviderId = mutableMapOf<Long, List<com.zenlemon.domain.model.Category>>()
    ...
    private fun resolveProviderIdMap(
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.zenlemon.domain.model.Provider>
    ...
    private suspend fun restorePlaybackHistory(
        history: List<com.zenlemon.domain.model.PlaybackHistory>,
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.zenlemon.domain.model.Provider>,
        conflictStrategy: BackupConflictStrategy
    ) {
...
internal fun com.zenlemon.domain.model.RecordingItem.toScheduledRecordingBackup(
...
internal suspend fun importScheduledRecordingBackups(
    ...
    storedProviders: List<ProviderEntity>,
    existingSchedules: MutableList<com.zenlemon.domain.model.RecordingItem>,
    conflictStrategy: BackupConflictStrategy,
    recordingManager: RecordingManager,
...
private fun com.zenlemon.domain.model.Provider.toSecureEntityForBackup(
...
private val PROVIDER_LIST_TYPE: Type = object : TypeToken<List<com.zenlemon.domain.model.Provider>>() {}.type
private val FAVORITE_LIST_TYPE: Type = object : TypeToken<List<com.zenlemon.domain.model.Favorite>>() {}.type
private val VIRTUAL_GROUP_LIST_TYPE: Type = object : TypeToken<List<com.zenlemon.domain.model.VirtualGroup>>() {}.type
private val PLAYBACK_HISTORY_LIST_TYPE: Type = object : TypeToken<List<com.zenlemon.domain.model.PlaybackHistory>>() {}.type
