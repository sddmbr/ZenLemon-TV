package com.zenlemon.data.sync

import android.content.Context
import android.util.Log
import com.zenlemon.data.local.DatabaseTransactionRunner
import com.zenlemon.data.local.dao.CatalogSyncDao
import com.zenlemon.data.local.dao.CategoryDao
import com.zenlemon.data.local.dao.ChannelDao
import com.zenlemon.data.local.dao.EpisodeDao
import com.zenlemon.data.local.dao.MovieCategoryHydrationDao
import com.zenlemon.data.local.dao.MovieDao
import com.zenlemon.data.local.dao.ProgramDao
import com.zenlemon.data.local.dao.ProviderDao
import com.zenlemon.data.local.dao.SeriesCategoryHydrationDao
import com.zenlemon.data.local.dao.SeriesDao
import com.zenlemon.data.local.dao.TmdbIdentityDao
import com.zenlemon.data.local.dao.XtreamContentIndexDao
import com.zenlemon.data.local.dao.XtreamIndexJobDao
import com.zenlemon.data.local.dao.XtreamLiveOnboardingDao
import com.zenlemon.data.local.entity.CategoryEntity
import com.zenlemon.data.local.entity.ChannelEntity
import com.zenlemon.data.local.entity.ChannelGuideSyncEntity
import com.zenlemon.data.local.entity.MovieEntity
import com.zenlemon.data.local.entity.SeriesEntity
import com.zenlemon.data.local.entity.XtreamContentIndexEntity
...
import com.zenlemon.data.parser.M3uParser
import com.zenlemon.data.remote.http.buildAppRequestProfile
import com.zenlemon.data.remote.http.toGenericRequestProfile
import com.zenlemon.data.remote.stalker.StalkerApiService
import com.zenlemon.data.remote.stalker.StalkerPlaybackMode
import com.zenlemon.data.remote.stalker.StalkerProvider
import com.zenlemon.data.remote.stalker.StalkerProviderProfile
import com.zenlemon.data.remote.jellyfin.JellyfinProvider
import com.zenlemon.data.remote.stalker.StalkerTrafficCoordinator
import com.zenlemon.data.remote.dto.XtreamCategory
import com.zenlemon.data.remote.dto.XtreamSeriesItem
import com.zenlemon.data.remote.dto.XtreamStream
import com.zenlemon.data.remote.xtream.OkHttpXtreamApiService
import com.zenlemon.data.remote.xtream.XtreamAuthenticationException
import com.zenlemon.data.remote.xtream.XtreamNetworkException
import com.zenlemon.data.remote.xtream.XtreamParsingException
import com.zenlemon.data.remote.xtream.XtreamRequestException
import com.zenlemon.data.remote.xtream.XtreamResponseTooLargeException
import com.zenlemon.data.remote.xtream.XtreamApiService
import com.zenlemon.data.remote.xtream.XtreamProvider
import com.zenlemon.data.remote.xtream.XtreamUrlFactory
import com.zenlemon.data.security.CredentialCrypto
import com.zenlemon.data.util.AdultContentClassifier
import com.zenlemon.data.util.UrlSecurityPolicy
...
import com.zenlemon.domain.sync.Section
import com.zenlemon.domain.sync.SyncProgress
...
class SyncManager @Inject constructor(
    ...
    ): com.zenlemon.domain.model.Result<Unit> {
        val providerEntity = providerDao.getById(providerId)
            ?: return com.zenlemon.domain.model.Result.error("Provider $providerId not found")
        ...
        return if (providerEntity.type == ProviderType.STALKER_PORTAL) {
            ...
            withProviderLock(providerId) {
                val freshProviderEntity = providerDao.getById(providerId)
                    ?: return@withProviderLock com.zenlemon.domain.model.Result.error("Provider $providerId not found")
    ...
    private suspend fun syncEpgLocked(
        providerEntity: com.zenlemon.data.local.entity.ProviderEntity,
        force: Boolean,
        ...
        if (!force && provider.type == ProviderType.STALKER_PORTAL && hasPendingStalkerCatalogIndex(provider.id)) {
            ...
            return com.zenlemon.domain.model.Result.success(Unit)
        }
        ...
        return try {
            ...
            com.zenlemon.domain.model.Result.success(Unit)
            ...
            com.zenlemon.domain.model.Result.error(safeMessage, e)
        }
    }
    ...
    ): com.zenlemon.domain.model.Result<Unit> = withProviderLock(providerId) lock@{
        try {
            val providerEntity = providerDao.getById(providerId)
                ?: return@lock com.zenlemon.domain.model.Result.error("Provider $providerId not found")
    ...
    suspend fun rebuildXtreamIndex(
        ...
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.zenlemon.domain.model.Result.error("Index rebuild is only available for Xtream providers")
        }
    ...
    private suspend fun syncXtreamIndexFirst(
        ...
        syncProgressBus.emit(
            com.zenlemon.domain.sync.SyncProgress(
                section = com.zenlemon.domain.sync.Section.VOD,
                current = 0,
                total = 0,
                currentLabel = "",
        ...
        syncProgressBus.emit(
            com.zenlemon.domain.sync.SyncProgress(
                section = com.zenlemon.domain.sync.Section.SERIES,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = liveCount
    ...
    suspend fun processQueuedXtreamIndexJobs(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return@lock com.zenlemon.domain.model.Result.error("Provider $providerId not found")
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.zenlemon.domain.model.Result.success(Unit)
        }
        ...
        if (warnings.isNotEmpty()) {
            ...
            com.zenlemon.domain.model.Result.error(warnings.first(), exception)
        } else {
            com.zenlemon.domain.model.Result.success(Unit)
        }
    }
    ...
    private suspend fun processStalkerSummaryIndexSection(
        ...
        data class StalkerCategoryFetchResult(
            ...
            val result: Result<com.zenlemon.data.remote.stalker.StalkerPagedResult<out Any>>
        )
            ...
            when (val result = fetchStalkerWildcardSummaryPageWithRecovery(api, contentType, wildcardCategory.categoryId, nextPage)) {
                is Result.Success -> {
                    ...
                    val anomaly = detectStalkerPageAnomaly(
                        hydration = currentHydration,
                        requestedPage = nextPage,
                        pagedResult = com.zenlemon.data.remote.stalker.StalkerPagedResult(
                            items = visibleItems,
    ...
    private suspend fun markStalkerAttemptStarted(
        ...
        when (contentType) {
            ContentType.MOVIE -> movieCategoryHydrationDao.upsert(
                com.zenlemon.data.local.entity.MovieCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
            ...
            ContentType.SERIES -> seriesCategoryHydrationDao.upsert(
                com.zenlemon.data.local.entity.SeriesCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
                    lastHydratedAt = hydration?.lastHydratedAt ?: 0L,
                    itemCount = hydration?.itemCount ?: 0,
    ...
    private suspend fun fetchStalkerSummaryPageWithRecovery(
        ...
        categoryId: Long,
        page: Int
    ): Result<com.zenlemon.data.remote.stalker.StalkerPagedResult<out Any>> {
    ...
    private suspend fun fetchStalkerSummaryFirstPageFallback(
        ...
        contentType: ContentType,
        categoryId: Long
    ): Result<com.zenlemon.data.remote.stalker.StalkerPagedResult<out Any>> = when (contentType) {
        ContentType.MOVIE -> when (val result = api.getVodStreams(categoryId)) {
            is Result.Success -> Result.success(
                com.zenlemon.data.remote.stalker.StalkerPagedResult(
                    items = result.data,
                    page = 1,
                    totalPages = 1,
    ...
    private fun detectStalkerPageAnomaly(
        hydration: StalkerHydrationSnapshot?,
        requestedPage: Int,
        pagedResult: com.zenlemon.data.remote.stalker.StalkerPagedResult<out Any>,
        pageFingerprint: String?
    ): String? {
    ...
    private suspend fun syncStalker(
        ...
        if (force || ContentCachePolicy.shouldRefresh(metadata.lastMovieSuccess, ContentCachePolicy.CATALOG_TTL_MILLIS, now)) {
            ...
            val categories = when (val categoriesResult = api.getVodCategories()) {
                is com.zenlemon.domain.model.Result.Success -> categoriesResult.data
                ...
                is com.zenlemon.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
            }
            emitCatalogSyncProgress(
                section = Section.VOD,
    ...
    private suspend fun syncJellyfin(
        ...
        try {
            ...
            val movieEntities = (resolvedMovies as? com.zenlemon.domain.model.Result.Success)?.data.orEmpty()
            val seriesEntities = (resolvedSeries as? com.zenlemon.domain.model.Result.Success)?.data.orEmpty()
            ...
            if (movieEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.zenlemon.domain.model.ContentType.MOVIE.name, listOf(
                        com.zenlemon.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 1L, name = "Movies",
                            type = com.zenlemon.domain.model.ContentType.MOVIE
                        )
                    ))
                }
            }
            ...
            if (seriesEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.zenlemon.domain.model.ContentType.SERIES.name, listOf(
                        com.zenlemon.data.local.entity.CategoryEntity(
    ...
    private suspend fun syncStalkerPortalEpg(
        ...
        val insertBuffer = ArrayList<com.zenlemon.data.local.entity.ProgramEntity>(STALKER_GUIDE_PROGRAM_BATCH_SIZE)
        ...
        fallbackGuideRequests.forEachIndexed { index, request ->
            ...
            runCatching {
                ...
                if (streamResult is com.zenlemon.domain.model.Result.Error) {
    ...
    private suspend fun syncXtreamLiveCatalog(
        ...
        syncProgressBus.emit(
            com.zenlemon.domain.sync.SyncProgress(
                section = com.zenlemon.domain.sync.Section.LIVE,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = 0
    ...
    private suspend fun stalkerCatalogAccessDiagnostic(
        ...
        val profile = when (val profileResult = api.getAccountProfile()) {
            is com.zenlemon.domain.model.Result.Success -> profileResult.data
            else -> return null
        }
    ...
    private fun <T> requireResult(result: com.zenlemon.domain.model.Result<T>, fallbackMessage: String): T {
        return when (result) {
            is com.zenlemon.domain.model.Result.Success -> result.data
            is com.zenlemon.domain.model.Result.Error -> throw IllegalStateException(result.message.ifBlank { fallbackMessage }, result.exception)
            is com.zenlemon.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
    }
