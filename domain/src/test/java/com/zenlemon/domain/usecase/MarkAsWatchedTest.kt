...
class MarkAsWatchedTest {
    ...
    private class FakePlaybackHistoryRepository : PlaybackHistoryRepository {
        ...
        override suspend fun markAsWatched(history: PlaybackHistory): com.zenlemon.domain.model.Result<Unit> {
            lastMarkedHistory = history
            return com.zenlemon.domain.model.Result.success(Unit)
        }
        override suspend fun recordPlayback(history: PlaybackHistory) = com.zenlemon.domain.model.Result.success(Unit)
        override suspend fun updateResumePosition(history: PlaybackHistory) = com.zenlemon.domain.model.Result.success(Unit)
        override suspend fun flushPendingProgress() = com.zenlemon.domain.model.Result.success(Unit)
        override suspend fun removeFromHistory(contentId: Long, contentType: ContentType, providerId: Long) = com.zenlemon.domain.model.Result.success(Unit)
        override suspend fun clearAllHistory() = com.zenlemon.domain.model.Result.success(Unit)
        override suspend fun clearHistoryForProvider(providerId: Long) = com.zenlemon.domain.model.Result.success(Unit)
        override suspend fun clearLiveHistoryForProvider(providerId: Long) = com.zenlemon.domain.model.Result.success(Unit)
    }
}
