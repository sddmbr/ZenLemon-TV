...
## ZenLemon emulator orientation

When starting the emulator with ZenLemon, always ensure the visible device frame
...
```bash
mkdir -p /private/tmp/zenlemon_live_validation
for i in $(seq -w 0 60); do
  adb exec-out screencap -p > /private/tmp/zenlemon_live_validation/freq_${i}.png
  stat -f "freq_${i} %z" /private/tmp/zenlemon_live_validation/freq_${i}.png
  sleep 2
done
```
...
```bash
shasum -a 256 /private/tmp/zenlemon_live_validation/freq_*.png | awk '{print $1}' | sort | uniq | wc -l
```

Then confirm the player is still healthy:

```bash
adb shell dumpsys media_session | awk '/package=com.zenlemon.app/{seen=1} seen && /metadata:/{print; getline; print; getline; print} seen && /state=PlaybackState/{print; exit}'
adb logcat -d -v time > /private/tmp/zenlemon_live_validation.log
rg -n "fatal-error|live-recovery selected|live-recovery no-candidate|prepare resolvedStreamType=MPEG_TS_LIVE|source-malformed live-ts-fallback|Player stuck|state=ERROR" /private/tmp/zenlemon_live_validation.log
rg -n "retry category=|first-frame-success|prepare resolvedStreamType=HLS|read-progress streamType=HLS" /private/tmp/zenlemon_live_validation.log | tail -80
```
