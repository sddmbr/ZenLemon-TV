...
ZenLemon plugins are companion Android APKs. They do not inject code into the
main application. Instead, ZenLemon discovers installed APKs that expose a
bound service with the action `com.zenlemon.plugin.API` and talks to them
...
ZenLemon from a declarative schema, or opened as a native plugin Activity when
...
```xml
<action android:name="com.zenlemon.plugin.API" />
```
...
- Any manual Android package install. ZenLemon detects it after refreshing the
  Plugins screen.

## Manifest

ZenLemon first asks the plugin service for its manifest. As a fallback, it
reads `com.zenlemon.plugin.MANIFEST_JSON` service metadata, then individual
...
```json
{
  ...
  "versionName": "1.0.0",
  "versionCode": 1,
  "description": "Adds external capabilities to ZenLemon.",
...
```xml
<meta-data android:name="com.zenlemon.plugin.ID" android:value="com.example.plugin" />
<meta-data android:name="com.zenlemon.plugin.NAME" android:value="Example Plugin" />
<meta-data android:name="com.zenlemon.plugin.VERSION_NAME" android:value="1.0.0" />
<meta-data android:name="com.zenlemon.plugin.VERSION_CODE" android:value="1" />
<meta-data android:name="com.zenlemon.plugin.DESCRIPTION" android:value="Adds external capabilities to ZenLemon." />
<meta-data android:name="com.zenlemon.plugin.PROVIDER_NAME" android:value="Example Provider" />
<meta-data android:name="com.zenlemon.plugin.CONFIGURATION_MODE" android:value="host.schema" />
<meta-data android:name="com.zenlemon.plugin.CONFIGURATION_ACTIVITY_ACTION" android:value="" />
<meta-data android:name="com.zenlemon.plugin.CAPABILITIES" android:value="provider.m3u,playback.prepare,cast.rewriteUrl,configuration.schema" />
```
...
- `provider.m3u`: the plugin can expose an M3U URL that ZenLemon imports as a
...
`handled=false` when the URL is not theirs. ZenLemon then continues with other
enabled plugins or the original URL.

`MSG_REWRITE_CAST_URL` is Cast-specific. ZenLemon sends the stream context that
...
`MSG_PREPARE_PLAYBACK` may also enrich the stream that ZenLemon sends to the
player. When `handled=true` and `success=true`, the plugin can return:

- `output_url`: final playable URL. If omitted, ZenLemon keeps `input_url`.
...
For `SMOOTH_STREAMING` + `clearkey`, ZenLemon normalizes the ISML
...
simple actions. ZenLemon owns the visual shell, validation placement, focus
...
`configurationMode` is `activity`, ZenLemon treats the Activity as the active
...
ZenLemon owns the visual implementation.
...
```json
{
  ...
  "description": "Settings rendered by ZenLemon.",
  "sections": [
    {
      "id": "connection",
      "title": "Connection",
...
- `info`: read-only text rendered by ZenLemon.
...
For `MSG_SET_CONFIGURATION_VALUES`, ZenLemon sends the same JSON object in
...
For `MSG_RUN_CONFIGURATION_ACTION`, ZenLemon sends `configuration_action_id`.
...
ZenLemon starts `configurationActivityAction` with the plugin package set
...
Native plugin Activities should feel at home when launched from ZenLemon and
...
- Design TV-first: every important control must be D-pad focusable and readable
  from couch distance.
- Keep the style restrained and close to ZenLemon: dark background, compact
...
- Test both entry points: launched directly from the plugin APK and launched from
  ZenLemon's Plugins screen.
- Test on the same device classes ZenLemon supports, including TV devices such
...
- Expose exactly one `com.zenlemon.plugin.API` service.
